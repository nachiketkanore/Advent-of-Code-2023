...
int apply_old_logic(string S) {
	map<char, int> freq;
	for (char ch : S) {
		freq[ch]++;
	}
	assert(freq.count('J') == 0);
	vector<int> F;
	for (auto [k, v] : freq) {
		F.push_back(v);
	}
...
int apply_new_logic(string S) {
	assert(count(ALL(S), 'J') > 0);
	int best_type = 10;
	for (char ch : cards) {
		if (ch != 'J') {
			string T = S;
			for (char& x : T)
				if (x == 'J')
					x = ch;
			int type = apply_old_logic(T);
			best_type = min(best_type, type);
		}
	}
	assert(best_type != 10);
	return best_type;
...
int get_type(string S) {
	...
	if (freq.count('J') > 0) {
		return apply_new_logic(S);
	} else {
		return apply_old_logic(S);
	}
}
